...
export default class FirestoneScene extends Phaser.Scene {
  constructor ()
  {
    super({key: 'Firestone'});
    Phaser.Scene.call(this, 'FirestoneScene');
  ...
  HandleQuestion()
  {
    ...
    .text(50, 25, "For this sorting algorithm, if the input array is in ascending order, then the total number of compares is linear.", {
      font: "16px monospace",
      fill: "#000000",
      padding: { x: 5, y: 5 },
  ...
  preload()
  {
    ...
    this.load.tilemapTiledJSON("firestonemap", "/iwgame/assets/firestone.json");

    this.load.image("kernighan", "/iwgame/assets/kernighan.png");
  }

  create()
  {
    // Make the this.Map
    this.Map = this.make.tilemap({ key: "firestonemap" });
    ...
    this.Opponent = this.physics.add.sprite(this.OpponentPoint.x, this.OpponentPoint.y, 'kernighan').setSize(16, 16).setOffset(0,0).setImmovable(true);
